...
public class BluetoothProbe extends Base implements PassiveProbe {
	...
	private BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(BluetoothDevice.ACTION_FOUND)) {
				JsonObject JsonObject = getGson().toJsonTree(intent.getExtras()).getAsJsonObject();
				JsonElement clazz = JsonObject.get("android.bluetooth.device.extra.CLASS");
				JsonElement bssid = JsonObject.get("android.bluetooth.device.extra.DEVICE");
				JsonElement name = JsonObject.get("android.bluetooth.device.extra.NAME");
				JsonElement rssi = JsonObject.get("android.bluetooth.device.extra.RSSI");

				JsonObject bluetoothElement = new JsonObject();

				try {
					String deviceClass = (clazz != null) ? clazz.toString().substring(clazz.toString().indexOf(":") + 1,
							clazz.toString().length() - 1) : "";
					bluetoothElement.addProperty(DatabaseHelper.TABLE_BLUETOOTH_CLASS, deviceClass);

					String deviceBssid = (bssid != null) ? bssid.toString().substring(bssid.toString().indexOf(":") + 2,
							bssid.toString().length() - 2) : "";
					bluetoothElement.addProperty(DatabaseHelper.TABLE_BLUETOOTH_BSSID, deviceBssid);

					String nameString = (name != null) ? name.getAsString() : "";
					bluetoothElement.addProperty(DatabaseHelper.TABLE_BLUETOOTH_DEVICE, nameString);

					String rssiString = (rssi != null) ? rssi.getAsString() : "";
					bluetoothElement.addProperty(DatabaseHelper.TABLE_BLUETOOTH_RSSI, rssiString);
				} catch (Exception e) {
					e.printStackTrace();
				}

				sendData(bluetoothElement);
	...
	private boolean shouldDisableOnFinish = false; // Keeps track of previous
													// bluetooth state
