...
public interface Probe {
	...
	public abstract class Base implements Probe, BaseProbeKeys {
		...
		protected void sendData(final JsonObject data) {
			...
			data.addProperty(DatabaseHelper.TABLE_STANDARD_DEVICEUID, android_id);
		...
		protected static final int ENABLE_MESSAGE = 1, START_MESSAGE = 2, STOP_MESSAGE = 3, DISABLE_MESSAGE = 4, SEND_DATA_MESSAGE = 5,
		...
		/**
		 * Sensitive data is hidden by default This can not be changed by
		 * configuration alone If you uncomment this line please submit the
		 * change to funf@media.mit.edu in accordance with the LGPL license. *
		 */
		// @Configurable
		...
		/**
		 * Used to override the serialiazation technique for multiple types. You
		 * can override this method to have getGson() return a Gson object that
		 * includes your TypeAdapterFactory.
		 * 
		 * @return
		 */
		...
		/**
		 * Return true if this should be wake locked while in running. For most
		 * probes this will return true, but some may need to allow the device
		 * to sleep.
		 * 
		 * @return
		 */
